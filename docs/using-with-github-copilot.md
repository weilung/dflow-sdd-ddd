...
`dflow configure-agents --skills` 目前只為 **Claude Code 與 Codex** 投影
project-level skill；在 **PROPOSAL-056 Phase 1** **不會**為 GitHub Copilot 另外
建立 `.github/skills/`，對 Copilot 執行時會印出一行延後說明。

但這**不代表 Copilot 不能用 skill**：實測（2026-06-05）顯示 Copilot 仍能 engage
Dflow 的 skill，只是觸發方式依介面而異——**VS Code Chat 自然語言自動觸發**、
**Copilot CLI 需打 `/dflow` 手動喚起**（細節見上方介面 A / B）。是否要為 Copilot
正式投影一份 native skill 仍是待評估項、尚無確定時程。
