...
### Skill 自動觸發在 Copilot 上的現況（目前延後）

若你看過 `dflow configure-agents --skills`（找回自然語言自動觸發的 skill），
請注意：在 **PROPOSAL-056 Phase 1**，`--skills` 只為 **Claude Code 與 Codex**
投影 project-level skill，**不會**為 GitHub Copilot 建立 `.github/skills`；執行時
會印出一行延後說明。Copilot 的 skill 自動觸發**目前延後**，尚無確定時程。
在此之前，Copilot 使用者請改用上方的 prompt adapters（`dflow configure-agents
--command-adapters`，產生 `.github/prompts/dflow-*.prompt.md`）作為工具原生入口。
