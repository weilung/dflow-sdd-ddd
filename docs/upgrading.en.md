...
|---|---|---|---|---|
| Starter scaffolding and your specs | `_overview.md`, the body of `_conventions.md`, `Git-principles-*.md`, everything you wrote under `dflow/specs/` | **You** | Untouched; the single exception is advancing the `> Dflow Version:` reconciliation line in `_conventions.md` to the current CLI version | — |
...
| Shim regions in agent files + the canonical region of the guide | `agent-shim` marker blocks inside `CLAUDE.md` / `AGENTS.md` / `.github/copilot-instructions.md`; the `guide-canonical` region of `AI-AGENT-GUIDE.md` | Dflow (inside markers) / you (outside) | **Refreshes the `agent-shim` and `guide-canonical` regions in place**; everything outside — including `## Project Context` — is preserved | — |
...
One-sentence version: **flagless refreshes the bundle plus the `agent-shim` and `guide-canonical` regions; command adapters (including the `codex-command-triggers` region in `AGENTS.md`) and existing skills each need their flag; content you wrote is never rewritten or migrated automatically.**
...
- the `Git-principles-{policy}.md` starter for your selected Git policy being edited or missing
