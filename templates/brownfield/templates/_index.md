...
Template note (for AI):
  ...
  Minimal HOST usage (zero-phase) — a different shape, not a smaller one:
    ...
    (references/finish-feature-flow.md Step 1). A baseline capture uses this
    same zero-phase shape, with a `Tier = baseline` row.
-->
...
> **baseline capture 的表長這樣，而且只有這一列**（bf 專屬）——closeout 對
> baseline host 要求 Lightweight Changes 表**恰好一列**，所以它不會和上面的
> T2／T3 列並存：
>
> | Date | Tier | Description | Commit |
> |---|---|---|---|
> | {YYYY-MM-DD} | baseline | 盤點 Expense 既有行為並寫入 BC 層 — `dflow/specs/domain/expense/rules.md`、`behavior.md` | {hash} |
