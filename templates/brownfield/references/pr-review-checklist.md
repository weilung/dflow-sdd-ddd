...
- [ ] **No-BR family evidence** — for a lightweight-spec whose Behavior
      ...
      **N/A** for these specs — record them as N/A rather than passing
      ...
      automatically N/A: a contract change that touches documented
      behavior still belongs in `behavior.md`. A spec in the
...
- [ ] **BC layer sync landed** — `dflow/specs/domain/{context}/rules.md` and
      ...
      `_index.md` Current BR Snapshot). **N/A for a no-BC host** — one whose
      Goals & Scope declares no bounded context has nothing to sync, and a
      `{context}` document invented to satisfy this line is the fiction
      closeout already blocks. **N/A for a baseline capture** — a baseline
...
- [ ] **Hosted `Commit` cell identity — confirm each hash is *that row's own*
      ...
      baseline row alongside phase work; `finish-feature-flow.md` says so where
      it explains why such a host never reaches the zero-phase checks.)
      **If the row declares nothing, that is by design on a hosted row — with
      ...
      row. For a **T3**, and for a **baseline** row, the absence is the
      sanctioned state — and for the baseline it is the same hosted exemption
      that covers a hosted T3, which `templates/_index.md` extends to baseline
      rows in the same breath. It is not a property of captures.
...
- **Accepted with rationale** — state why the boundary / transaction / service
  ...
  must already exist or be added to the appropriate Aggregate / Domain Service
  row and Notes in `models.md`; when it protects an invariant, update the
  related `rules.md` entry too. The PR conversation points to that decision and
  is not its only home. This disposition evaluates the rationale's adequacy,
  not only its existence: it must explain how it answers every matched risk.
  For a match involving an external provider / service fact, acknowledge that
  a local transaction cannot make that fact atomic with local state and explain
  how the outcome progresses across the boundary, including idempotency and
  retry / compensation (or a justified equivalent). For a Domain Service match,
  name the actual domain rule or decision the accepted operation carries; an
...
- the existing durable surfaces that must change, as applicable — the spec,
  domain `models.md` / `rules.md` / `context.md` / `context-map.md` /
  `behavior.md`, or `migration/tech-debt.md`;
...
exit criteria, and `migration/tech-debt.md` landing surface as closure
