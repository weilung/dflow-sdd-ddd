...
> tier, and Part B has done the host linkage. An entry that arrived *here* ran
> neither, so recovering into one of them skips the authoritative cascade and
> the completed-feature scan — which is how a change that should have been a
> follow-up gets archived as an unlinked standalone. Step 1 routes it from
> there, and its Part B hotfix callout is what sends a **T2 / T3 post-hoc**
> to Step 1.8 — the only route that attaches `reconciled
