# Post-Hoc Hotfix Reconciliation — Greenfield Clean Architecture
...
- **A tracked delta (T3 or above), minimal (zero-phase) host** — it does
  **not** go into this host: the two-commit lifecycle has one implementation
  ...
  delta is routed **after** Step 4 archives it — by the **A tracked delta (T3
  or above)** bullet of § After closeout, the same route every post-closeout
  leftover takes. Do **not** route it now:
  you are mid-closeout, and routing would open a second host before this one
  is sealed. Note it, finish the closeout, then take § After closeout's route —
