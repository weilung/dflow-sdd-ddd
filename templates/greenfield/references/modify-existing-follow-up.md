# Step 1.6: Create Follow-up Feature — Greenfield Clean Architecture
...
rather than stand beside it. On that tier: a **T1** follow-up takes the full
new-feature machinery below; a **T2 / T3** follow-up takes
the **minimal (zero-phase) variant** described after the reverse-link step —
it does **not** delegate new-feature Step 4's unconditional first phase-spec.
A **post-hoc hotfix** whose host linkage Step 1.8 resolved to *follow-up*
also arrives here (never through the Step 1.5 prompt, and always T2 / T3, so
always the minimal variant); its first checkpoint keeps the `implementation`
name but carries Result `reconciled (...)` per Step 1.8.
...
"Add follow-up reference to `{新 SPEC-ID}`"). For the **minimal (T2 / T3)
variant** the AI does **not** offer a separate host-open commit — the
initial reverse-link edit rides along in the implementation checkpoint (see
the variant paragraph below), keeping the follow-up at exactly two host
checkpoints. The reverse link is a derived index — the new feature's
...
**Minimal (zero-phase) follow-up variant (T2 / T3).** When the follow-up
change is itself lightweight, do **not** delegate new-feature Step 4 (which
would create an unconditional first phase-spec). Instead treat the follow-up
host exactly like a standalone minimal host — open it via **Step 1.7's
...
or more T3 inline rows, or both for a compound follow-up (Step 1.7's
"minimal means zero-phase, not one-artifact") — and close with
`/dflow:finish-feature`. The lifecycle is **two host checkpoints**
(implementation, then closeout) **plus one post-closeout reverse-link flip
tracking commit** — the flip (the original feature's Follow-up Tracking row
