...
- [ ] **Minimal host (zero-phase) only** — **nothing that constitutes the change
      ...
      **May ride into the closeout commit — this list and nothing else:**
      ...
      `Implementation Tasks` collapse / removal the Step 5 completion checklist
      ...
      *after* checkpoint 1, so this edit is uncommitted here by construction;
      ...
      delta. A **T3** has no Domain sweep at all (Step 5's tier-conditional
...
- **(i) BC-bearing** — the host touched a real bounded context, **whether or
  ...
  **`Aggregates affected:` and `Domain Events Changes:` take what this host
  ...
  reader of this summary has no other source for the difference. Step 5's
  "exact fields" block states the same shape for a zero-phase host and remains
  the authority on that host's *whole* field set; this sentence is the half that
  was only ever written there.
...
"phase-spec" references in the steps below (events "introduced by phase-specs",
"each phase-spec's Delta") as the host's **lightweight-spec** recorded delta
...
reach every host shape, and its phase-bearing exemption note confirms those hosts
keep them. So a phase-bearing host can hold a hosted T2 whose delta belongs in
...
- a **no-BC host** (Step 3 case ii) sets the fields that report a **sync** to
...
**Step 3 case (i)**. The two say the same thing on purpose; if you change one,
change the other.
...
- **The resolution moves system state** — if choosing between the two versions
  changes a business rule, a documented behaviour, an event, or the domain
  model, the owning document (`rules.md` / `behavior.md` / `events.md` /
  `models.md`) **must** be updated. That update is itself a tracked change and
  takes the route above; system-state truth is never below workflow.
