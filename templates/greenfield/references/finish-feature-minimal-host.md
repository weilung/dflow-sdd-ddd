...
- [ ] **Minimal host (zero-phase) only** — **the Checkpoint Log carries exactly
      ...
      **blocks** — **report the one that is true**, because the repairs differ.
      ...
      `implementation`, a leftover from an abandoned attempt, or one this list
      does not name — says the host did not take the two-commit lifecycle, and
      that is settled with the developer, not by deleting a line. A failed or
      declined attempt does **not** add a row: both are recorded by editing the
