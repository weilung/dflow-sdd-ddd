...
- `CreateExpenseReportCommand` + Handler + Validator — 一次帶入整份 Draft 的 Items，
  內部逐筆呼叫 Aggregate 的 `AddItem()`（BR-003 / BR-004 仍由 Domain 層守住）
...
### Presentation Layer
- `ExpenseReportsController` — `POST /api/expense-reports`（建 Draft，含 Items）、`POST /api/expense-reports/{id}/submit`
- Request / Response 模型
- Swagger annotation
...
### Domain Unit Tests
- [x] ExpenseReport.Submit() 主場景：3 個 Item → Status = Submitted、event raised、TotalAmount 正確
- [x] ExpenseReport.Submit() 空 Items → throw DomainException（驗 BR-001）
- [x] Submitted 後 AddItem → throw DomainException（驗 BR-002）
- [x] Money(0, "TWD") → throw ArgumentException（驗 BR-003 / EC-001）
- [x] AddItem 重複 ReceiptReference → throw DomainException（驗 BR-004 / EC-002）
- [x] 二次 Submit → throw DomainException（驗 EC-003）

### Application Tests
- [x] SubmitExpenseReportCommandHandler 主場景（mock IExpenseReportRepository）
- [x] Validator 驗 ReportId / EmployeeId 非空

### Integration Tests
- [x] ExpenseReportRepository 存取 round-trip（用 in-memory SQLite 或 Testcontainers）
...
- [x] DOMAIN-1: 建 `Money` / `ReceiptReference` / `ExpenseCategory` 三個 VO（record types，建構式驗證）
- [x] DOMAIN-2: 建 `ExpenseItem` Entity（金額 > 0 驗證；private setters）
- [x] DOMAIN-3: 建 `ExpenseReport` Aggregate Root（Status enum、`_items` 集合、`Submit()` / `AddItem()` / `RemoveItem()` / `ModifyItem()` 方法、`DomainEvents` 集合）
- [x] DOMAIN-4: 建 `ExpenseReportSubmitted` Domain Event (record)
- [x] DOMAIN-5: 建 `IExpenseReportRepository` interface
- [x] APP-1: 建 `CreateExpenseReportCommand` + Handler + Validator（直接帶入完整 Items）
- [x] APP-2: 建 `SubmitExpenseReportCommand` + Handler + Validator
- [x] APP-3: 建 DTOs (`ExpenseReportDto`、`ExpenseItemDto`)
- [x] INFRA-1: 建 `ExpenseReportConfiguration` (EF Fluent API)，含 Owned Types
- [x] INFRA-2: 實作 `ExpenseReportRepository`
- [x] INFRA-3: 產 `InitialExpenseSchema` migration
- [x] API-1: 建 `ExpenseReportsController` + 兩個 endpoints + Request/Response models + Swagger
- [x] TEST-1: Domain unit tests — 6 個案例（見 Test Strategy § Domain Unit Tests）
- [x] TEST-2: Application handler tests — Submit handler 主場景 + Validator 測試
- [x] TEST-3: Integration test — Repository round-trip
