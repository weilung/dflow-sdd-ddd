...
- phase 2 如何更新 aggregate design、domain rules、models、events、glossary 與 context-map
...
上一篇 [〈Walkthrough 02 — `/dflow:new-feature` 建立第一個 Expense feature〉](walkthrough-02-new-feature.zh-TW.md)
...
```text
Dflow:
收到。/dflow:new-phase 是「在既有 active feature 裡新增 phase-spec」的流程，
...
## Step 5 — Sync living docs

Dflow 同步更新 living docs：
...
- aggregate-design.md：
  新增 ApprovalDecision，更新 ExpenseReport 狀態機，標 phase-2 ADDED / MODIFIED
- dflow/specs/domain/Expense/models.md：
  新增 ApprovalDecision / ApprovalReason / enum
- dflow/specs/domain/Expense/rules.md：
  MODIFIED BR-002，ADDED BR-005..007
- dflow/specs/domain/Expense/events.md：
  新增 ExpenseReportApproved / ExpenseReportRejected
- dflow/specs/domain/Expense/context.md：
  Approval BC open question 改成已決定留在 Expense BC
- dflow/specs/domain/glossary.md：
  Approver 升級為已建模參與者，新增 ApprovalDecision / ApprovalReason
- dflow/specs/domain/context-map.md：
  補 external Identity reference，
  因為 BR-005 讓 SubmitterId / ApproverId 關係成為明確依賴

不建 behavior.md。
per skill 規定，behavior scenarios 仍由 phase-spec 承載，
等 finish-feature / Step 8.3 才 merge 到 BC-level behavior.md。
```
...
```markdown
## Aggregate: ApprovalDecision

ApprovalDecision 記錄主管針對一次 Submit 做出的審核決定。
它不是 ExpenseReport 的 child entity，
因為它有自己的稽核識別、唯一約束與生命週期；
同一份 ExpenseReport 被退回後重新 Submit，
會產生新的 SubmitAttemptNo，
並對應新的 ApprovalDecision。
```
...
| BR-007 | Reject 必須附註原因；ApprovalReason 至少 10 字元，否則 raise DomainException。 | ApprovalDecision | active | 2026-04-29 |
```

完整文件範例：
[`outputs/dflow/specs/domain/Expense/rules.md`](outputs/dflow/specs/domain/Expense/rules.md)

補充：完整文件範例的 BR-007 已包含後續 T2 lightweight bilingual length change。本篇 phase 2
當下先以「至少 10 字元」建立規則。
...
|---|---|---|---|---|
| ApprovalDecision | ApprovalDecision | Approver 不可等於 Submitter；Rejected 必須有至少 10 字元原因；同一 Submit attempt 最多一筆 decision | `ExpenseTracker.Domain.Expense.ApprovalDecision` | 第二個 Aggregate；一個 Submit attempt 對應一筆 decision |
```
...
|---|---|---|---|---|---|
| ExpenseReportApproved | ExpenseReport (Aggregate) | `ExpenseReport.Approve()` 成功且 ApprovalDecision 建立後 raise | `(ExpenseReportId, SubmitterId, ApproverId, ApprovedAt, SubmitAttemptNo)` | (phase 3 Reimbursement 預期 consumer；phase 2 暫無外部 consumer) | in-process |
| ExpenseReportRejected | ExpenseReport (Aggregate) | `ExpenseReport.Reject()` 成功且 ApprovalDecision 建立後 raise | `(ExpenseReportId, SubmitterId, ApproverId, RejectedAt, Reason, SubmitAttemptNo)` | UI read model / future notification handler | in-process |
```

完整文件範例：
[`outputs/dflow/specs/domain/Expense/events.md`](outputs/dflow/specs/domain/Expense/events.md)

## Step 5 — Branch timing
...
這裡的重點不是 branch name，而是順序：spec 與 BR Delta 先落地，implementation commit 後開始。
...
| Spec-first development | phase 2 spec / Delta / aggregate design 先落地，再開始 implementation branch。 |
| Hybrid workflow control | Dflow 在 phase slug、phase spec draft、`_index.md` refresh 前後都設 phase gate。 |
| DDD semantic backbone | ApprovalDecision 的 Aggregate 身分、Expense BC 歸屬、Identity external reference 都被顯式記錄。 |
| 三層文件分工 | phase spec 管 phase delta，feature `_index.md` 管 current snapshot，domain docs 管 BC current model。 |
...
- `/dflow:new-phase` 在既有 active feature 內新增 phase-spec，不新建 feature directory。
...
- `_index.md` Current BR Snapshot 是 current state，Step 5 regenerate，不是歷史流水帳。
- Living docs 要同步：aggregate design、rules、models、events、glossary、context-map。
- Behavior scenarios 在 phase 2 當下仍由 phase spec 承載；不要硬建不存在的 `behavior.md`。

## 下一個 walkthrough

下一個 Greenfield walkthrough 可接 [〈Walkthrough 04 — `/dflow:modify-existing` 調整 BR-007 reject reason 長度〉](walkthrough-04-modify-existing.zh-TW.md)：
