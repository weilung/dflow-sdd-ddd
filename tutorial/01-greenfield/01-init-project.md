...
>  - `AGENTS.md` / `CLAUDE.md` / `GEMINI.md` / `.github/copilot-instructions.md`: not present → 等等可選擇建立對應 AI tool shim
>
...
> **Dflow CLI**：收到。這次會使用 Greenfield track scaffolding。
>
> **Q2 tech stack**：ASP.NET Core 版本？ORM？Mediator？測試框架？.NET / C# 版本？
...
>
> **Q3 migration context**：這個專案是從別的 stack 移植過來，還是純新 build？如果是移植，`_overview.md` 會加 migration context note。
...
> **Alice**：`_overview.md` 要。Git 我們已經決定採 trunk-based 了（單一 main + 短命 feature branch + squash merge），所以挑 `Git-principles-trunk.md`，gitflow 那份不要。
...
> **Alice**：四個都建。團隊主要用 Codex，但有人會用 Claude Code、Gemini 和 GitHub Copilot。
...
> | Q5 optional | `_overview.md` ✓ / `Git-principles-trunk.md` ✓ |
> | Q6 AI agents | `AGENTS.md` ✓ / `CLAUDE.md` ✓ / `GEMINI.md` ✓ / `.github/copilot-instructions.md` ✓ |
>
...
> | `AGENTS.md` (repo root) | selected tool shim |
> | `CLAUDE.md` (repo root) | selected tool shim |
> | `GEMINI.md` (repo root) | selected tool shim |
> | `.github/copilot-instructions.md` | selected tool shim |
>
...
> - 寫 `dflow/specs/architecture/tech-debt.md`（從 `templates/tech-debt.md`，標題改 ExpenseTracker，欄位用 Greenfield track layer 標籤）
...
>
> 4.4 special — AI agent instruction files：
> - 寫 `dflow/specs/shared/AI-AGENT-GUIDE.md` 作為 canonical project guide
> - repo root 沒有 `AGENTS.md` / `CLAUDE.md` / `GEMINI.md`，`.github/copilot-instructions.md` 也不存在 → 建立四份 thin shim
> - 每份 shim 只宣告本專案使用 Dflow，並指向 `dflow/specs/shared/AI-AGENT-GUIDE.md`
> - `CLAUDE.md` / `GEMINI.md` 額外包含 Markdown import hint：`@dflow/specs/shared/AI-AGENT-GUIDE.md`
>
> 寫檔過程：
...
> - ✓ `dflow/specs/shared/AI-AGENT-GUIDE.md` — canonical AI agent guide
> - ✓ `AGENTS.md` （repo root） — tool shim
> - ✓ `CLAUDE.md` （repo root） — tool shim
> - ✓ `GEMINI.md` （repo root） — tool shim
> - ✓ `.github/copilot-instructions.md` — tool shim
>
...
>  - `dflow/specs/shared/AI-AGENT-GUIDE.md`：若團隊後續決定新增專案級 AI 協作規則，集中補在這份，不要分散到各 tool shim
>
...
`npx dflow-sdd-ddd init` Step 4 在 Alice 的 ExpenseTracker repo 寫入了 **15 個檔**，按目錄分組如下：
...
- [`dflow/specs/architecture/tech-debt.md`](outputs/dflow/specs/architecture/tech-debt.md) — 架構債 backlog（templates/tech-debt.md，Greenfield track layer 標籤）
...
**shared/ 專案級治理檔（Q5 optional + Q6 AI guide）**
...
- [`dflow/specs/shared/AI-AGENT-GUIDE.md`](outputs/dflow/specs/shared/AI-AGENT-GUIDE.md) — AI tool-neutral canonical guide，所有 tool shim 都指回這份

**AI tool shims**
- [`AGENTS.md`](outputs/AGENTS.md) — Codex / Copilot coding agent shim，指向 canonical guide
- [`CLAUDE.md`](outputs/CLAUDE.md) — Claude Code shim，指向 canonical guide 並包含 Markdown import hint
- [`GEMINI.md`](outputs/GEMINI.md) — Gemini CLI shim，指向 canonical guide 並包含 Markdown import hint
- [`.github/copilot-instructions.md`](outputs/.github/copilot-instructions.md) — GitHub Copilot repository instruction shim
...
- **Q5 optional 與 Q6 AI agents 分離**。Optional starter 只處理 `_overview.md` / Git principles；AI agent setup 另建 canonical `AI-AGENT-GUIDE.md` 和各工具 thin shim，避免 Dflow 公開版看起來只支援 Claude Code。
- **Q6 全選反映 multi-AI 團隊情境**。Alice 一次選 Codex、Claude Code、Gemini、GitHub Copilot，展示 Dflow 如何維持單一 source of truth，而不是四份規則各自漂移。同時 Dflow 在 Alice 勾完 trunk-based 後特別「reconfirm 不要 gitflow」，讓同事看到 CLI 對「容易誤勾」的選項會主動把關。
...
- **AI tool file special handling**：repo 沒既有 tool file → 直接建 root shim；若既有檔存在，Dflow 會改放 `dflow/specs/shared/*-snippet.md` 等 merge snippet，不覆寫團隊原本的規則。
- **不建 `behavior.md` 與 ADR-* 的設計理由**被明確 announce（per F-05、Day-0 too early），對應 init-project-flow.md §3.1 的 Greenfield-track notes 與 §3.3 not-creating 區塊。這是同事最容易困惑的「為什麼有些檔故意不建」，對話直接點明來由，不留疑問。
