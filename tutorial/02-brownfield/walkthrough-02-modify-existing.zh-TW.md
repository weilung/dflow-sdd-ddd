...
本篇來源是 [02-modify-existing.md](02-modify-existing.md)，但不是單純重排原始對話。
它會把 Bob 與 Dflow 的互動改寫成一份可教學、可 review 的讀物，讓讀者看懂：
...
部分檔案在後續 `03-baseline-capture.md`、`04-new-feature.md`、`05-bug-fix.md`、
`06-finish-feature.md` 會再被修改。因此，本篇內嵌的 code block 代表本步驟結束當下的
重點片段；連結則提供讀者檢查最終完整文件。
...
[note] final snapshot 已包含後續 baseline capture、BUG-001 rounding inconsistency、
closeout integration summary；它比本步驟當下版本更完整。
...
[note] final snapshot 還包含後續 `SPEC-20260505-002` 加入的 VIP discount BR-005~008。
...
[note] 上表連結是 final snapshot。後續 closeout 會把 feature 移到 `completed/`，
並加入 BUG-001 與 integration summary；本篇關注的是 step 02 建立這些文件的第一個版本。
...
| Three-layer documentation model | phase spec 捕捉本次修改；feature `_index.md` 捕捉 snapshot；domain docs 捕捉長期 Order rules。 |
...
下一個 Brownfield 教學可接 [03-baseline-capture.md](03-baseline-capture.md)：
