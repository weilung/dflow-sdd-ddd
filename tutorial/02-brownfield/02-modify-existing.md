...
> **Bob**：這段不要引入 Application layer。Brownfield track 先 Domain + Code-Behind adapter + tests 就好。

> **Dflow**：同意。這是 Brownfield track，不是 Greenfield track。Implementation Tasks 我會用 Brownfield 適合的分類：
>
...
**故意不建 / 不改**
- 不建 `aggregate-design.md` — Brownfield track 用 `models.md` 描述 Aggregate，`aggregate-design.md` 是 Greenfield 劇情慣例。
...
- **no host feature → escalate to new-feature**：雖然 Bob 入口是 `/dflow:modify-existing`，但 Step 1 判定為 T1 Heavy，且沒有 active/completed host feature，所以依 Brownfield modify-existing flow 轉入 `/dflow:new-feature`。
