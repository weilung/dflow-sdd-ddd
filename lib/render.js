...
function completedYearLinksHtml(groups) {
  const links = completedKeyOrder(groups.keys()).map((key) => {
    ...
    return `<a href="${escapeHtml(completedIndexRel(key))}">${escapeHtml(completedKeyLabel(key))} (${g.units.size})</a>`;
  });
...
async function renderIndex({ outRoot, relFiles, title, stamp, completedGroups }) {
  let tree;
  if (completedGroups) {
  ...
  const body = `<h1>${escapeHtml(title)}</h1>${tree}`;
...
async function renderCompletedIndex({ outRoot, key, groups, title, stamp }) {
  ...
  const nav = completedKeyOrder(groups.keys())
    .map((k) => (k === key
      ...
      : `<a href="${escapeHtml(`index-${k}.html`)}">${escapeHtml(completedKeyLabel(k))}</a>`))
    .join(' · ');
  const body =
...
async function runRender({ cwd, args = [], stdout, stderr }) {
  ...
  if (completedGroups) {
    for (const key of completedKeyOrder(completedGroups.keys())) {
      written.push(await renderCompletedIndex({ outRoot, key, groups: completedGroups, title: options.title, stamp }));
    }
  }
  written.push(await renderIndex({ outRoot, relFiles: relMdFiles, title: options.title, stamp, completedGroups }));
