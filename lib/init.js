...
async function checkInitOnlyStarters(cwd, findings) {
  ...
  if (conventionsAbsent || !conventions.trim()) {
    ...
    // ⚠ SAY THIS BEFORE THE DRIFT FINDINGS, because it changes what they mean.
    // An HTML block of types 1-5 closes only on its end condition, so a single
    // unclosed `<!--` left mid-edit puts the whole rest of the file inside it.
    // `conventionsSectionBodies` now stops at that line rather than treating the
    // block's interior as section content — which is correct, and which makes
    // every section below it look empty. Without this finding the user gets a
    // `stale` that is right and inexplicable.
    //
    // It is deliberately NOT a `drift` entry: the drift renderer maps `state`
    // through a fixed lookup, so a new state would render `undefined` as both
    // its title and its detail. This is a document-level condition anyway, not
    // a per-fingerprint one.
    const unclosedAt = doctorChecks.unclosedHtmlBlockLine(conventions);
    if (unclosedAt !== -1) {
      findings.push({
        level: 'warn',
        title: `dflow/specs/shared/_conventions.md has an unclosed HTML block at line ${unclosedAt}`,
        detail: `Everything from that line to the end of the file is inside that block, so it is not section content and Dflow cannot assess drift below it. Any "missing" or "is missing the rule" finding below this one may be caused by the unclosed block rather than by real drift.`,
