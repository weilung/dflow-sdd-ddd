...
async function checkInitOnlyStarters(cwd, findings) {
  ...
  if (policy) {
    ...
    // ⚠⚠ THE COMPARISON USES `resolvedEdition`, NOT `edition`. Reporting drift
    // means asserting "your canonical sections differ from the packaged ones",
    // and that sentence names a specific packaged file — so it may only be said
    // when we know WHICH track this project is. `p090-b3-y1` executed the gap:
    // with the manifest forced to brownfield on a structurally greenfield
    // project, doctor compared the greenfield project against the BROWNFIELD
    // starter and reported `canonical sections differ`; `configure-agents` then
    // corrected the manifest and the next doctor run was clean. The drift never
    // existed — it was a resolver disagreement wearing a content finding.
    // ⚠ The damage-reporting above deliberately does NOT narrow this way: a
    // broken package is worth saying whichever track you turn out to be, while
    // a content comparison is not. Different claims, different evidence bars.
    const projectPackaged = resolvedEdition ? packagedByEdition.get(resolvedEdition) : null;
    const packagedUsable = Boolean(projectPackaged && projectPackaged.usable);
    ...
    if (!projectFileExists) {
      ...
      if (region.state === 'malformed') {
        ...
        findings.push({
          ...
          action: 'Repair or remove the stray `<!-- dflow-generated: git-principles-canonical ... -->` markers, then re-run `dflow configure-agents`.'
        });
        ...
        if (unreadable) {
          ...
          findings.push({
            ...
            action: 'Re-run `dflow configure-agents` on an interactive terminal and accept the marker-adoption offer; it keeps your file header and everything from "## 6. AI Collaboration Rules (Project Policy)" down.'
          });
        } else {
          findings.push({
            level: 'info',
      ...
      } else if (packagedUsable) {
        const projectedCanonical = toLf(projected).slice(region.startIdx, region.endIdx);
        const packagedCanonical = toLf(projectPackaged.template).slice(projectPackaged.region.startIdx, projectPackaged.region.endIdx);
        if (projectedCanonical !== packagedCanonical) {
          findings.push({
            level: 'info',
            title: `${relativePath} canonical sections differ from the current packaged starter`,
            detail: 'Only sections 1-5 are compared; your own sections are not. This is either your edit inside the managed region, or a starter that has not been refreshed yet.',
            action: 'Run `dflow configure-agents` to refresh the canonical sections in place; content outside the markers is kept.'
          });
        }
      }
    }
