...
async function addAiAgentShim(cwd, items, agent, substitution, options = {}) {
  ...
  if (isPristineDflowAgentsShim(existingContent, baseShimBody)) {
...
// PROPOSAL-056 Phase 1: project-level skill paths per AI agent. Claude and Codex
// (the `agents` key, labeled "Codex / Copilot coding agent") each get the SAME
// edition-neutral thin skill projected to their own canonical skill path. The
// `copilot` key is DEFERRED in Phase 1 — `.github/skills` is not projected; a
// clear deferral note is printed instead. De-dupe holds for this scope: claude
// writes only `.claude/skills`, agents writes only `.agents/skills`, and each
// tool reads only its own path, so a project with both has no duplicate `dflow`
// skill (the multi-read duplicate case only arises with Copilot, deferred).
...
async function addSkillAdapterItems(cwd, items, aiAgents, skills, warnings) {
  ...
  if (aiAgents.includes('copilot')) {
    // Phase 1 defers Copilot skill projection (PROPOSAL-056). Do not create
    // .github/skills; surface a clear note so the omission is intentional and
    // visible rather than a silent gap.
    warnings.push(
      'Copilot skill projection is deferred (PROPOSAL-056 Phase 1); no .github/skills adapter was generated for GitHub Copilot.'
    );
  }
  ...
  if (skillTargets.length === 0) {
    // No skill-capable agent (claude / agents) was selected — e.g. only Copilot,
    // which is deferred above, or no agents at all. Nothing to project.
    if (!aiAgents.includes('copilot')) {
      warnings.push(
        'The --skills flag projects a project-level skill for Claude Code and Codex; no skill adapter was generated because neither was a selected target.'
      );
    }
    return;
  }
