...
async function inferTechStackSummary(cwd) {
  ...
  const match = content.match(doctorChecks.TECH_STACK_ROW_RE);
...
async function inferMigrationContext(cwd) {
  const overviewPath = path.join(cwd, 'dflow/specs/shared/_overview.md');
  const content = await fs.readFile(overviewPath, 'utf8').catch(() => '');
  const match = content.match(doctorChecks.MIGRATION_CONTEXT_ROW_RE);
...
function isRecognizableDflowGuide(lfContent) {
  return /^# Dflow AI Agent Guide\s*$/m.test(lfContent) && /^## Project Context\s*$/m.test(lfContent);
}
...
function projectContextSectionBounds(lfContent) {
  ...
  const start = lines.findIndex((line) => /^## Project Context\s*$/.test(line));
  if (start < 0) return null;
  let end = lines.length;
  for (let i = start + 1; i < lines.length; i += 1) {
    if (/^## /.test(lines[i]) || lines[i].startsWith(GUIDE_CANONICAL_SECTION_START)) {
      end = i;
      break;
    }
  }
...
async function checkInitOnlyStarters(cwd, findings) {
  ...
  // No _overview.md machine-format check: the current packaged _overview
  // template carries no `| Tech stack |` / `| Migration / legacy context |`
  // rows, so their absence is the canonical state, not drift. (The context
  // inference that reads those rows never finds them on a current-template
  // project either — a pre-existing inference-source gap, tracked outside
  // PROPOSAL-058.)
}
