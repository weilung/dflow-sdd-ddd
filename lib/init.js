...
async function addAiAgentShim(cwd, items, agent, substitution) {
  ...
  const targetExists = await pathExists(path.join(cwd, target.relativePath));
  const relativePath = targetExists ? target.snippetPath : target.relativePath;
  const content = substitutePlaceholders(buildAiAgentShim(target.relativePath), substitution);
  const notes = targetExists
    ? `selected, ${target.relativePath} already exists; merge this snippet manually`
    : 'selected, tool-specific shim';
